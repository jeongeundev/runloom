...
AUTH=Path(os.environ.get('GATELESS_PROBE_AUTH', str(Path.home()/'.codex/auth.json')))
TOKEN=secrets.token_urlsafe(32)
RUNS=[]

def docker(owner, args, name=None):
    result=['docker','run','--rm','--user',f'{os.getuid()}:{os.getgid()}','--cap-drop','ALL','--security-opt','no-new-privileges','--read-only','--tmpfs','/tmp:rw,nosuid,nodev,mode=1777','--tmpfs',f'/home/node:rw,nosuid,nodev,uid={os.getuid()},gid={os.getgid()},mode=700','--mount',f'type=bind,src={AUTH},dst=/home/node/auth.json,readonly','--mount',f'type=bind,src={ROOT/owner},dst=/workspace','--env','CODEX_HOME=/home/node','--env','HOME=/home/node']
