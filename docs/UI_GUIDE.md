...
> **조건부 문서** — 사용자 인터페이스가 있는 프로젝트에만 해당한다.
> CLI·데이터 파이프라인·라이브러리처럼 UI 가 없는 프로젝트에서는 이 파일을 삭제한다.
> (`docs/*.md` 는 매 step 프롬프트에 주입되므로, 빈 채로 두면 컨텍스트만 낭비한다.)
> 아래 클래스 예시는 Tailwind CSS 기준이며, 다른 스타일링 도구를 쓰면 표기만 바꾼다.
