...
2. 홈 → `시연 업무 만들기` 로 A(진단, 자동 완료) 를 등록하고 `실행`. `실행 요청됨 → 실행 중 → 완료 · 판정 근거: n/m` 을 본다.
3. A 상세의 `후속 업무 B 등록` 으로 B(코드 수정, 자동 실행, 검토 후 완료) 를 등록한다. A 가 이미 완료면 워커 스캔(3초) 후 B 가 자동 착수한다. Codex 실행은 2~3분(Step 15 기준).
