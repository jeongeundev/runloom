...
5. **AC는 실행 가능한 커맨드** — "~가 동작해야 한다" 같은 추상적 서술이 아닌, 그대로 복사해 실행할 수 있는 커맨드를 포함한다. 커맨드는 `CLAUDE.md`의 명령어 섹션에 정의된 것을 그대로 쓴다. PRD의 품질 기준에 해당 step과 관련된 항목이 있으면 그것도 실행 가능한 검증으로 AC에 넣는다.
...
```bash
{빌드}     # 컴파일/빌드 에러 없음 — CLAUDE.md 명령어 섹션 값
{테스트}   # 테스트 통과 — CLAUDE.md 명령어 섹션 값
